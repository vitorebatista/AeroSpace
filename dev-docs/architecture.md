...
- `../Sources/AppBundle/`.
  AeroSpace-edge.app server. Technically, it's a SPM library that is exposed to `xcode-app-bundle-launcher`
- `../Sources/Cli/`.
...
`aerospace-edge` CLI binary is client. `AeroSpace-edge.app` is server. Client and server talk to each other via predefined UNIX file.

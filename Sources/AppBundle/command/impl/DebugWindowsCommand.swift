...
struct DebugWindowsCommand: Command {
    ...
    func run(_ env: CmdEnv, _ io: CmdIo) async throws -> BinaryExitCode {
        ...
        if let appBundleId = args.appBundleId {
            return try await dumpAppWindowsDebugInfo(appBundleId, io)
        }
...
private func dumpWindowDebugInfo(_ window: Window) async throws -> String {
    ...
    result = result + (try await dumpAppDebugInfo(window.macApp))
    result = result + (try await dumpTreeNodeDebugInfo(window))
...
@MainActor
private func dumpAppDebugInfo(_ macApp: MacApp) async throws -> [String: Json] {
    ...
    result["Aero.AXApp"] = .dict(try await macApp.dumpAppAxInfo())
    return result
}

@MainActor
private func dumpTreeNodeDebugInfo(_ window: MacWindow) async throws -> [String: Json] {
    var result: [String: Json] = [:]
...
@MainActor
private func dumpAppWindowsDebugInfo(_ appBundleId: String, _ io: CmdIo) async throws -> BinaryExitCode {
    ...
    for app in apps {
        ...
        guard let appDump = try? await dumpAppDebugInfo(app),
              let dumps = try? await app.dumpAllWindowsAxInfo(windowLevels)
        else {
            return .fail(io.err("Failed to dump AX info for app-bundle-id '\(appBundleId)'. Please report to https://github.com/nikitabobko/AeroSpace/discussions/categories/potential-bugs"))
        }
        for (index, idAndDump) in dumps.enumerated() {
            ...
            if let windowId, let macWindow = MacWindow.allWindowsMap[windowId] {
                ...
                result = result + (try await dumpTreeNodeDebugInfo(macWindow))
            } else {

    ...
    if config.enableNormalizationFlattenContainers {
        ...
        if containsSplitCommand {
            errors += [.semantic(
                ...
                My recommendation: keep the normalizations enabled, and prefer 'join-with' over 'split'. Alternatively, disable the normalization on a single workspace with '\(aeroSpaceCliName) enable-normalization flatten-containers off'.
                """,
            )]
        }
    }

...
If you don't have a custom config yet, the window opens on <<default-config,the default
config>> and the first save writes it to `~/.aerospace-edge.toml`, together with whatever
you changed. The footer tells you so before you save.
...
as you type. That feedback checks the pane on its own, as a fragment, so it can't see
problems that only exist in the context of the whole file — Save is the authority, and it
can still reject a pane that reported itself fine.

Only what you actually change is written. A key, a table, or a pane you never touched is
left byte for byte as it was, so saving is never a reformat of the whole file.

Comments and key order in your config are preserved, with one exception: the `gaps`,
`key-mapping`, `exec`, and `workspace-to-monitor-force-assignment` tables — including their
sub-tables, such as `[gaps.inner]`, `[key-mapping.key-notation-to-key-code]`, and
`[exec.env-vars]` — are rewritten in full when one of their values changes, so comments
written inside those tables are lost. Comments everywhere else are kept.

If the config doesn't parse at all, there is no parsed config to build a form from, so the
window degrades to a single raw editor over the whole file with the parser's message above
it. Fix the file there and save: the save re-reads what it wrote, so once the file parses
the form comes back on its own.
...
* `AEROSPACE_WORKSPACE` and `AEROSPACE_WINDOW_ID` - describe the current focus. Exactly one is set:
  `AEROSPACE_WINDOW_ID` if a window is focused, otherwise `AEROSPACE_WORKSPACE` for an empty workspace.
  This lets the callback forward the focus to another `aerospace` command without a self-conflicting target.
...
When AeroSpace runs a callback's command (such as an <<on-window-detected-callback>> or an <<on-focus-changed-callbacks>>),
...
That makes commands in callbacks operate on the originating window or workspace even when global focus has already moved.
...
A common culprit is KeePassXC: at the time of writing, v2.7.10 can get stuck holding secure input mode (see: https://github.com/keepassxreboot/keepassxc/issues/11906[#11906]). The current workaround is to downgrade to v2.7.9. Other apps (e.g. Ferdium) can trigger the same problem, so if you don't use KeePassXC, check whether another running app is holding secure input.
...
* xref:./commands.adoc#subscribe[*subscribe command:*] (treated separately) The client sends one `ClientRequest` frame, then the server sends an unbounded stream of `ServerEvent` frames until the client disconnects. The client must not send anything else on the connection.
...
| `args`
| array of strings
| The CLI arguments as you would pass them to `aerospace`, *excluding* the program name itself. For example, `["workspace", "1"]` is the wire form of `aerospace workspace 1`. To send the `subscribe` command, pass `["subscribe", ...]` here.

| `stdin`
| string
| Contents to feed to the command as standard input. Use `""` if the command doesn't read stdin. Some commands (e.g. xref:./commands.adoc#workspace[`workspace --stdin`]) consume this field.

| `windowId`
| unsigned 32-bit integer or `null`
| The value of the `AEROSPACE_WINDOW_ID` environment variable as the client saw it, or `null` if there is no such variable in scope. AeroSpace sets this variable when invoking xref:commands.adoc#exec[exec callbacks] so that the called process knows the window it was triggered from. If you are calling AeroSpace from your own context (no surrounding window), send `null`.

...
- **Release build:** `./script/build-release.sh --build-version <ver> --codesign-identity aerospace-codesign-certificate`
  ...
  **Never release with `--codesign-identity -`.** Ad-hoc signatures have no certificate, so the
  designated requirement is the literal binary hash and macOS drops every user's Accessibility and
  Screen Recording grants on each update. `build-release.sh` now enforces this: it rejects an ad-hoc
  identity unless `--allow-adhoc` is passed, and checks the finished artifacts' designated
  requirement against the pinned `release_cert_leaf`. `script/create-codesign-certificate.sh` creates
  the certificate; see "Signing identity" in `dev-docs/fork-maintenance.md`. CI is the one exception
  (no keychain) and passes `--allow-adhoc` explicitly.
- **Homebrew cask:** every release must also be published to the fork's tap
  (`vitorebatista/homebrew-tap`, installed as `brew install --cask vitorebatista/tap/aerospace-edge`).
  `Casks/aerospace-edge.rb` is generated by `./script/build-brew-cask.sh` — never hand-edited.
  Steps in [`dev-docs/fork-maintenance.md`](./dev-docs/fork-maintenance.md) → "Updating the Homebrew tap".
